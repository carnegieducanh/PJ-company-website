...
/**
 * Tự động đánh dấu active cho sidenav
 * dựa trên URL trang hiện tại
 */
window.addEventListener("template-loaded", () => {
    const currentFile = window.location.pathname.split("/").pop();
    document.querySelectorAll(".sidenav-item__link").forEach((link) => {
        const href = link.getAttribute("href");
        if (href && href.split("/").pop() === currentFile) {
            link.closest(".sidenav__list-item")?.classList.add("current-active");
        }
    });
});
